...
function notifychangedmessages(notification) {

	var card = notification['card'];
	var changedmessages = notification['changedmessages'];

	var body = "";
	body += "<div style=\"padding-left:65px;\">";
	for (var i in changedmessages) {

		body += "<div style=\"font-weight:bold;font-size:14pt;color:#333;\">";
		body += htmlEncode(changedmessages[i]['from']) + "&nbsp;&nbsp;&gt;&nbsp;&nbsp;" + htmlEncode(card['cloudname']);
		body += "</div>";
		body += "<div style=\"font-size:10pt;color:#999;margin-bottom:5px;\">";
		body += htmlEncode(changedmessages[i]['content']);
		body += "</div>";
	}
	body += "</div>";
...
appAPI.ready(function($) {
	...
	appAPI.message.addListener(function(msg) {
		...
		if (msg.type === 'notifychangedvalues') notifychangedvalues(msg.notification);
		if (msg.type === 'notifychangedmessages') notifychangedmessages(msg.notification);
	});
});
